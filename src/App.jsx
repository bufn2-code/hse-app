...
export default function App() {
  ...
  const safeAreas = masterData?.areas?.length > 0 ? masterData.areas : defaultSettings.areas;
  const safeRoles = masterData?.roles?.length > 0 ? masterData.roles : defaultSettings.roles;
  ...
  useEffect(() => {
    if (activeTab === 'dashboard' && dashboardMode === 'tahunan' && personnel.length > 0) calculateYearlyBest();
  ...
  const handleAddArea = () => {
    if(!newArea.trim()) return; const formattedArea = newArea.trim().replace(/\b\w/g, l => l.toUpperCase());
  ...
  const getAccumulatedData = (empId, role) => {
    ...
    Object.values(empData).forEach(weekData => { getActiveCategories(role).forEach(c => { if(c?.key) total[c.key] += (weekData[c.key] || 0); }); });
    return total;
  };
  ...
  const calculateScore = (acc, um, roleId) => {
    ...
    targetedCats.forEach(c => { const val = acc[c?.key] || 0; if (c?.target && val < c.target) { sAwal -= (((c.target - val) / c.target) * weightPerCat); } });
    if(sAwal < 0) sAwal = 0; 

    let tPoin = Number(um?.kepatuhan) || 75; untargetedCats.forEach(c => { if(c?.key) tPoin += (acc[c.key] || 0) }); 
    ...
    try {
      for (let m = 1; m <= 12; m++) {
        ...
        personnel.filter(p => p.role === selectedRoleContext).forEach(p => {
          ...
          Object.values(empWeekly).forEach(weekData => { getActiveCategories(p.role).forEach(c => { if(c?.key) totalWeeklyAcc[c.key] += (weekData[c.key] || 0); }); });
  ...
  const getDefisitTarget = () => {
    ...
    targetPersonnel.forEach(p => {
      ...
      getActiveCategories(p.role).filter(c => c?.isTargeted).forEach(c => {
        ...
        if (c?.target && tercapai < c.target) { defisit.push({ id: p.id + c.key, nama: p.nama, area: p.area, indikator: c.label, tercapai, target: c.target, kurang: c.target - tercapai }); }
      });
    });
